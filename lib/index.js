...
async function look(session, item, ctx, config, logger) {
  try {
...
__name(look, "look");
async function search(session, item, ctx, config, logger) {
...
async function apply(ctx, config) {
  ...
  ctx.command("mcwiki.search <item>", "搜索某个关键词").action(({ session }, item) => search(session, item, ctx, config, logger));
  ctx.command("mcwiki.look <item>", "查看某个页面").action(({ session }, item) => look(session, item, ctx, config, logger));
